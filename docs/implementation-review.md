...
This is the historical local implementation review. Source links were converted to repository-relative links for publication; the findings and verification record remain those of the initial checkpoint. Any later Fable review of a PR is a separate review of its recorded commit.
...
[main.rs:78](../crates/axiom-core/src/main.rs#L78) compiles an accepted build spec, serializes the complete bundle, and writes it with exclusive creation at lines 85–92, without checking its serialized size. Every later file input is capped at 16 MiB by [read_text](../crates/axiom-core/src/main.rs#L16). The bundle contains the source strings plus manifest and executable, so an accepted source spec can expand beyond that cap.
