...
**Proposal v1 · 4 September 2026 · Prepared for Max Ghenis · Pending Fable review**
...
Object writes are immutable and idempotent. PostgreSQL stores search/navigation projections and an index of these objects. A serving pointer is mutable, but activation checks the expected predecessor, verified object availability, and admission policy in one transaction. Concurrent activations cannot silently overwrite each other. A transactional outbox records activation events for dependent workers.
...
Compilation checks imports, cycles, types, branch compatibility, available parameter versions, units, and entity/period constraints that are statically decidable. Input-dependent constraints remain runtime checks. A successful compile promises conformance to those executable invariants, not substantive legal correctness.
